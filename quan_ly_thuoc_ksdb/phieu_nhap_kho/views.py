...
def danh_sach_phieu_nhap(request):
    ...
    so_phieu_nhap = request.GET.get('so_phieu_nhap')
...
def nhap_thuoc(request):
    form = NhapThuocForm(request.POST or None)
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect('phieu_nhap_kho')

    items_danh_muc = Thuoc.objects.all()
    return render(request, 'phieu_nhap_kho.html', {'form': form, 'items_danh_muc': items_danh_muc})
